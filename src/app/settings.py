...
class Settings(BaseSettings):
    ...
    pdf_deplot_model: str | None = Field(
        default="google/deplot",
        validation_alias=AliasChoices("PDF_DEPLOT_MODEL", "pdf_deplot_model"),
    )

    table_enable_structure: bool = Field(
        default=True,
        validation_alias=AliasChoices("TABLE_ENABLE_STRUCTURE", "table_enable_structure"),
    )
    figure_enable_deplot: bool = Field(
        default=True,
        validation_alias=AliasChoices("FIGURE_ENABLE_DEPLOT", "figure_enable_deplot"),
    )
    figure_deplot_gate: bool = Field(
        default=True,
        validation_alias=AliasChoices("FIGURE_DEPLOT_GATE", "figure_deplot_gate"),
    )
    deplot_min_rel_area: float = Field(
        default=0.04,  # require figure bbox >= 4% of page area
        validation_alias=AliasChoices("DEPLOT_MIN_REL_AREA", "deplot_min_rel_area"),
    )
    deplot_max_rel_area: float = Field(
        default=0.6,   # and <= 60% of page area
        validation_alias=AliasChoices("DEPLOT_MAX_REL_AREA", "deplot_max_rel_area"),
    )
    deplot_max_local_spans: int = Field(
        default=30,    # allow only if local text density (span count) is low
        validation_alias=AliasChoices("DEPLOT_MAX_LOCAL_SPANS", "deplot_max_local_spans"),
    )
    ...
    # PDF processing concurrency settings
    pdf_render_max_workers: int = Field(
        default=4,
        validation_alias=AliasChoices("PDF_RENDER_MAX_WORKERS", "pdf_render_max_workers"),
        description="Number of threads for parallel page rendering"
    )
    pdf_process_max_workers: int = Field(
        default=2,
        validation_alias=AliasChoices("PDF_PROCESS_MAX_WORKERS", "pdf_process_max_workers"),
        description="Number of threads for model inference (layout/OCR)"
    )
    pdf_max_concurrent_pages: int = Field(
        default=8,
        validation_alias=AliasChoices("PDF_MAX_CONCURRENT_PAGES", "pdf_max_concurrent_pages"),
        description="Maximum pages to process concurrently (memory limit)"
    )
    torch_num_threads: int = Field(
        default=0,
        validation_alias=AliasChoices("TORCH_NUM_THREADS", "torch_num_threads"),
        description="PyTorch thread count (0 = use PyTorch defaults, 1 = single thread for MPS)"
    )
    
    # Batch processing settings for GPU operations
    layout_batch_size: int = Field(
        default=8,
        validation_alias=AliasChoices("LAYOUT_BATCH_SIZE", "layout_batch_size"),
        description="Batch size for layout detection"
    )
    ocr_batch_size: int = Field(
        default=16,
        validation_alias=AliasChoices("OCR_BATCH_SIZE", "ocr_batch_size"),
        description="Batch size for OCR processing"
    )
    table_batch_size: int = Field(
        default=8,
        validation_alias=AliasChoices("TABLE_BATCH_SIZE", "table_batch_size"),
        description="Batch size for table structure detection"
    )
